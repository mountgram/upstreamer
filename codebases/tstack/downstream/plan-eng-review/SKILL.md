...
Before ANYTHING else in this skill — before Step 0 and any `git` / `Read` / `Grep` / `Glob` / `Bash` call — unless an exception below applies, your VERY FIRST tool call MUST be AskUserQuestion, to confirm the review target. Do not run any bash or explore the repo before the user answers.
