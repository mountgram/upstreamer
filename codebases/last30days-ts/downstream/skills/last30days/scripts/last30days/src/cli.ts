#!/usr/bin/env node
import { runResearch } from "./index.js";
import { renderMarkdown, renderJson, renderCompact } from "./render.js";
import { getConfig } from "./config.js";
import { getDateRange } from "./dates.js";
import type { QueryPlan, SourceItem } from "./schema.js";

interface CliArgs {
  topic: string;
  lookback?: number;
  depth?: "quick" | "medium" | "deep";
  timeframe?: "recent" | "all";
  format?: "markdown" | "json" | "compact";
  output?: string;
  debug?: boolean;
  xHandle?: string;
  subreddits?: string[];
  githubUser?: string;
  githubRepos?: string[];
  plan?: string;
  includeSources?: string[];
  webBackend?: string;
  hiringSignals?: boolean;
  jobBoard?: string;
}

function parseArgs(raw: string[]): CliArgs | null {
  const args = raw.slice(2);
  if (args.length === 0 || args[0] === "--help" || args[0] === "-h") {
    console.error("Usage: last30days <topic> [options]");
    console.error("");
    console.error("Options:");
    console.error("  --lookback <days>     Days to look back (default: 30)");
    console.error("  --timeframe <mode>    recent or all (default: recent)");
    console.error("  --depth <level>       quick, medium, or deep (default: medium)");
    console.error("  --format <type>       markdown, json, or compact (default: markdown)");
    console.error("  --output <dir>        Output directory (default: ./output)");
    console.error("  --debug               Enable debug output");
    console.error("  --x-handle <handle>   X/Twitter handle for person search");
    console.error("  --subreddits <list>   Comma-separated subreddits");
    console.error("  --github-user <user>  GitHub username for person search");
    console.error("  --github-repos <list> Comma-separated owner/repo");
    console.error("  --plan <file>         JSON query plan generated by an agent");
    console.error("  --include-sources <list> Comma-separated source list");
    console.error("  --web-backend <name>  openai or perplexity for opt-in LLM-grounded web search");
    console.error("  --hiring-signals      Analyze hiring patterns from jobs postings");
    console.error("  --job-board <name>     Company board name for job search");
    console.error("");
    console.error("Run `npx last30days-ts setup` to see source availability.");
    return null;
  }

  const result: CliArgs = { topic: args[0] };
  for (let i = 1; i < args.length; i++) {
    switch (args[i]) {
      case "--lookback":
        result.lookback = Number(args[++i]);
        break;
      case "--timeframe":
        result.timeframe = args[++i] as "recent" | "all";
        break;
      case "--depth":
        result.depth = args[++i] as "quick" | "medium" | "deep";
        break;
      case "--format":
        result.format = args[++i] as "markdown" | "json" | "compact";
        break;
      case "--output":
        result.output = args[++i];
        break;
      case "--debug":
        result.debug = true;
        break;
      case "--x-handle":
        result.xHandle = args[++i];
        break;
      case "--subreddits":
        result.subreddits = args[++i].split(",").map(s => s.trim());
        break;
      case "--github-user":
        result.githubUser = args[++i];
        break;
      case "--github-repos":
        result.githubRepos = args[++i].split(",").map(s => s.trim());
        break;
      case "--plan":
        result.plan = args[++i];
        break;
      case "--include-sources":
        result.includeSources = args[++i].split(",").map(s => s.trim()).filter(Boolean);
        break;
      case "--web-backend":
        result.webBackend = args[++i];
        break;
      case "--hiring-signals":
        result.hiringSignals = true;
        break;
      case "--job-board":
        result.jobBoard = args[++i];
        break;
    }
  }
  return result;
}

async function readPlan(file?: string): Promise<QueryPlan | undefined> {
  if (!file) return undefined;
  const fs = await import("node:fs/promises");
  const raw = await fs.readFile(file, "utf-8");
  const parsed = JSON.parse(raw) as QueryPlan;
  if (!parsed || !Array.isArray(parsed.subqueries)) {
    throw new Error(`Invalid query plan: ${file}`);
  }
  return parsed;
}

async function main(): Promise<void> {
  if (process.argv[2] === "setup") {
    const { printSetup } = await import("./setup.js");
    printSetup();
    return;
  }

  if (process.argv[2] === "source") {
    const sourceName = process.argv[3];
    const query = process.argv[4];
    if (!sourceName || !query) {
      console.error("Usage: last30days source <name> <query>");
      process.exit(1);
    }
    const report = await runResearch({
      topic: query,
      depth: "medium",
      includeSources: [sourceName],
      debug: true,
    });
    console.log(renderCompact(report));
    return;
  }

  const args = parseArgs(process.argv);
  if (!args) {
    process.exit(1);
  }

  const config = getConfig();

  if (args.debug) {
    console.error("Configuration loaded");
    console.error(`  Topic: ${args.topic}`);
    console.error(`  Lookback: ${args.lookback || 30} days`);
    console.error(`  Depth: ${args.depth || "medium"}`);
  }

  const report = await runResearch({
    topic: args.topic,
    lookbackDays: args.lookback || 30,
    depth: args.depth || "medium",
    timeframe: args.timeframe,
    debug: args.debug,
    xHandle: args.xHandle,
    subreddits: args.subreddits,
    githubUser: args.githubUser,
    githubRepos: args.githubRepos,
    outputDir: args.output || config.last30daysDir,
    outputFormat: args.format || "markdown",
    queryPlan: await readPlan(args.plan),
    includeSources: args.includeSources,
    webBackend: args.webBackend,
    hiringSignals: args.hiringSignals,
    jobBoard: args.jobBoard,
  });

  const format = args.format || "markdown";
  let output: string;
  switch (format) {
    case "json":
      output = renderJson(report);
      break;
    case "compact":
      output = renderCompact(report);
      break;
    default:
      output = renderMarkdown(report);
  }

  console.log(output);

  // Save to file if output dir specified
  const outDir = args.output || config.last30daysDir;
  if (outDir) {
    const fs = await import("node:fs/promises");
    const path = await import("node:path");
    const slug = args.topic.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "");
    const timestamp = new Date().toISOString().replace(/[:.]/g, "-").slice(0, 19);
    const ext = format === "json" ? "json" : "md";
    const filename = `${slug}-${timestamp}.${ext}`;

    try {
      await fs.mkdir(outDir, { recursive: true });
      await fs.writeFile(path.join(outDir, filename), output);
      if (args.debug) console.error(`Saved to ${path.join(outDir, filename)}`);
    } catch (err) {
      if (args.debug) console.error(`Failed to save output: ${err}`);
    }
  }
}

main().catch(err => {
  console.error("Error:", err.message);
  process.exit(1);
});
