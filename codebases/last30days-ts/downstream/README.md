...
└── scripts/
    └── last30days/
        ...
        │   ├── cli.ts
        │   ├── setup.ts
        │   └── sources/  (source SDKs and utilities)
        └── test/
```
...
Configure `EXA_API_KEY` for preferred web search or `BRAVE_API_KEY` as a fallback. Use `OPENAI_API_KEY` for optional OpenAI web-search grounding, and `GEMINI_API_KEY` for YouTube video understanding and Maps grounding. Weather, health, public jobs, GitHub, Reddit, Hacker News, Polymarket, and some local-binary sources can work without provider keys. Other source-specific keys are optional and isolated.
