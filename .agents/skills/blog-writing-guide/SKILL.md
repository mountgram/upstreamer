...
description: Write, review, and improve blog posts and technical write-ups for upstreamer following this repo's specific writing standards, voice, and quality bar. Use this skill whenever someone asks to write a blog post, draft a technical article, review blog content, improve a draft, write a project announcement, create an engineering deep-dive, or produce written content destined for upstreamer's developer and agent-builder audience. Also trigger when the user mentions "blog post," "blog draft," "write-up," "announcement post," "engineering post," "deep dive," "postmortem," or asks for help with technical writing for upstreamer. Even if the user just says "help me write about [feature/topic]", if it sounds like it could become an upstreamer technical post, use this skill.
---

# upstreamer Blog Writing Skill

This skill enforces upstreamer's writing standards across every post or write-up, whether you're helping an engineer explain a rewrite contract, document a generated downstream, or draft a project announcement.

**The bar:** Every upstreamer post should be something a senior engineer would share in their team's Slack, reference in a technical decision, or use when deciding how to keep a synthetic codebase synced.

What follows are the core principles to internalize and apply to every piece of content.

## The upstreamer Voice
...
Use "we" (upstreamer maintainers) and "you" (the reader). This is a conversation, not a paper.
...
**Bad:** "At upstreamer, we're always looking for ways to improve synthetic codebase workflows. Today, we're thrilled to share some exciting updates that we think you'll love."
...
- Bad: Personal intro → clinical middle → "Run upstreamer today."
...
**Honesty over hype.** Never overstate what a feature does. Acknowledge limitations. If something is experimental, say so. If another approach does something well, it's okay to note that. Do not claim AI-backed conversion is more capable than it is: "the converter preserved the workflow" is not the same as "the converter preserved every important behavior."
...
- "Performance improvements in upstreamer"
- "AI-powered codebase syncing with upstreamer"

## The Closing

End with something useful: a link to docs, source code, a command to run, or a call to give feedback. Never end with generic hype ("We can't wait to see what you build!"), recaps of what you just said, or product-page CTAs ("Run upstreamer today. It changes everything."). Connect back to the story you opened with, or give the reader something concrete to do next.
...
| Data / Research | Original insights from synthetic codebase syncs, converter logs, or evals | Engineering or research |
...
1. Never publish without a real person's name on it. No "The upstreamer Team" bylines.
